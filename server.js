...
const { saveBets, db } = require('./database');
...
app.get('/api/bets', (req, res) => {
    try {
        const rows = db.prepare('SELECT * FROM bets ORDER BY timestamp DESC').all();
        res.json({ bets: rows });
    } catch (err) {
...
app.delete('/api/delete-all-bets', (req, res) => {
    try {
        const result = db.prepare('DELETE FROM bets').run();
        res.json({ 
...
app.delete('/api/bets/:number', (req, res) => {
    ...
    try {
        const result = db.prepare('DELETE FROM bets WHERE number = ?').run(number);
        if (result.changes === 0) {
